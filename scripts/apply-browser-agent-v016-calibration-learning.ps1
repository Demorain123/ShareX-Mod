...
# ---------------------------------------------------------------------------
# Main UI: calibration button calls a real attached-tab benchmark. The normal
# main form can still hide during capture; a no-activate monitor shows live values.
# ---------------------------------------------------------------------------
Replace-One $main @'
...
# ---------------------------------------------------------------------------
# Capture session: load one bounded local profile, expose measured/effective
# values, learn from accepted frames, publish live telemetry, persist periodically.
# ---------------------------------------------------------------------------
Replace-One $session @'
...
'@ @'
                BrowserAgentAdaptiveDecision adaptiveDecision = adaptive.Observe(record);
                record.AdaptiveRiskScore = adaptiveDecision.RiskScore;
'@ 'BrowserAgentAdaptiveDecision adaptiveDecision = adaptive.Observe(record);' # marker already exists; no-op guard

# Inject after the complete adaptive decision fields, before logging.
Replace-One $session @'
...
# Frame payload measurements are deliberately separate from DOM/layout stability.
Replace-One $session @'
...
# ---------------------------------------------------------------------------
# Extension: enforce Chrome's capture rate limit, expose capture timing, add a
# benchmark command that returns numeric samples only, and measure false-quiet gaps.
# ---------------------------------------------------------------------------
Replace-One $worker @'
let nativePort = null;
let target = null;
...
'@ @'
    ...
    pngDataUrl
'@ 'captureThrottleWaitMs,`r`n    pngDataUrl'
...
'@ @'
            ...
            initialScrollHeight,
'@ 'maxFalseQuietMs: Math.round(maxFalseQuietMs),`r`n            initialScrollHeight'
...
# Diagnostics describe the new decision path explicitly.
if (-not $CheckOnly) {
...
# Extend deterministic self-test with local profile persistence/corruption handling.
