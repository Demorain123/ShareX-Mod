...
The dedicated `Build LongCapture Standalone` workflow applies the current ShareX-Mod overlay chain, publishes self-contained `win-x64`, verifies that the package contains `LongCapture.exe` and not `ShareX.exe`, runs the packaged-engine/GUI self-test, runs the ShareX-Mod semantic regression suite, rebuilds the ZIP, extracts it, and repeats the packaged self-test before upload.
