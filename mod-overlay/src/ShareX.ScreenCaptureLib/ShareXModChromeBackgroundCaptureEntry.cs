...
internal static class ShareXModChromeBackgroundCaptureEntry
{
    ...
    {
        if (!settings.ChromeEnhancedEnabled || !settings.ChromeBackgroundCapture || selectedWindowHandle == IntPtr.Zero)
        {
            return null;
        }
        ...
        try
        {
            ...
            ShareXModChromeBackgroundCaptureResult? result = await ShareXModChromeBackgroundCapture.CaptureAsync(
                client,
                target,
                settings,
                shouldStop);

            if (result != null && settings.ChromeImageAppendixEnabled)
            {
                try
                {
                    await ShareXModChromeImageAppendix.ExportAsync(client, settings, result.DirectoryPath);
                }
                catch
                {
                }
    ...
    private static ShareXModChromeTarget? SelectBestTarget(IReadOnlyList<ShareXModChromeTarget> targets, string windowTitle)
    {
        ...
        return targets
            ...
            .Select(t => new { Target = t, Score = CommonTitleScore(normalizedWindow, NormalizeTitle(t.Title)) })
    ...
    private static string NormalizeTitle(string value)
    {
        ...
        string[] suffixes = { " - Google Chrome", " – Google Chrome", " - Microsoft Edge", " – Microsoft Edge", " - Brave", " – Brave", " - Vivaldi", " – Vivaldi" };
