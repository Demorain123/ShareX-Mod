...
internal sealed class ShareXModSegmentStore : IDisposable
{
    ...
    public static ShareXModSegmentStore? TryCreate(ShareXModV04Settings settings) { if (!settings.SmartSegmentationEnabled) return null; try { return new ShareXModSegmentStore(settings); } catch { return null; } }
    ...
    private Bitmap CreatePreview(int sourceWidth, long sourceHeight)
    {
        ...
        g.Clear(Color.White); g.CompositingMode = CompositingMode.SourceCopy; g.InterpolationMode = InterpolationMode.HighQualityBicubic;
        int y = 0;
        foreach (PartInfo info in parts)
        {
            using Bitmap part = new(Path.Combine(directory, info.FileName));
            int ph = Math.Max(1, (int)Math.Round(part.Height * scale));
            g.DrawImage(part, new Rectangle(0, y, width, ph), new Rectangle(0, 0, part.Width, part.Height), GraphicsUnit.Pixel);
            y += ph;
        }
    ...
    {
        try { File.WriteAllText(Path.Combine(directory, "manifest.json"), JsonSerializer.Serialize(new { format="ShareX-Mod smart segmented capture", version="0.4.0-dev", final, width, height, partCount=parts.Count, parts, created=DateTimeOffset.Now }, new JsonSerializerOptions { WriteIndented=true })); } catch { }
    }
    ...
    private static string ResolveOutputDirectory(string configured)
    {
        ...
        try { Directory.CreateDirectory(primary); return primary; } catch { string fallback=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"ShareX-Mod","SegmentedCaptures"); Directory.CreateDirectory(fallback); return fallback; }
    }
...
internal static class ShareXModSegmentedOutputRegistry
{
    ...
    private static WeakReference<Bitmap>? previewRef; private static string? manifestPath; private static int sourceWidth; private static long sourceHeight;
    public static void Register(Bitmap preview,string manifest,int width,long height){lock(Sync){previewRef=new WeakReference<Bitmap>(preview);manifestPath=manifest;sourceWidth=width;sourceHeight=height;}}
    public static bool TryGet(Bitmap? bitmap,out string? manifest,out int width,out long height){lock(Sync){bool match=bitmap!=null&&previewRef!=null&&previewRef.TryGetTarget(out Bitmap? target)&&ReferenceEquals(target,bitmap);manifest=match?manifestPath:null;width=match?sourceWidth:0;height=match?sourceHeight:0;return match;}}
}
