...
$policyPath = Join-Path $repoRoot "LongCapture.Standalone\BrowserAgentIntegrityPolicyV018.cs"
$sessionPath = Join-Path $repoRoot "LongCapture.Standalone\BrowserAgentCaptureSession.cs"
...
if ($CheckOnly) {
    Write-Host "Browser Agent v0.1.8 integrity policy r4 compatibility passed." -ForegroundColor Green
    exit 0
}

[IO.File]::WriteAllText($policyPath, $policy, [Text.UTF8Encoding]::new($true))

# Perfect/High repair uses the most conservative recapture gear. This affects only
# the post-pass repair, not the user's first-pass High/Very High throughput.
...
$new = @'
        BrowserAgentFrameTuning repairTuning = BrowserAgentAdaptiveProfiles.ForGear(
            options.RepairPrecision == BrowserAgentRepairPrecision.Perfect ? 0 :
            options.RepairPrecision == BrowserAgentRepairPrecision.High ? 0 :
            options.RepairPrecision == BrowserAgentRepairPrecision.Medium ? 1 : 2);
'@
if (-not $session.Contains('BrowserAgentRepairPrecision.Perfect ? 0 :'))
{
    if (-not $session.Contains($old)) { throw "v0.1.8 integrity r4 repair-tuning anchor missing" }
    [IO.File]::WriteAllText($sessionPath, $session.Replace($old, $new), [Text.UTF8Encoding]::new($true))
}

Write-Host "Browser Agent v0.1.8 integrity policy r4 applied: robust scroll/anchor risk + conservative Perfect repair gear." -ForegroundColor Green
