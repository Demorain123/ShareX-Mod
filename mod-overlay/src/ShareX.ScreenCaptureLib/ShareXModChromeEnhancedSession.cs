...
internal sealed class ShareXModChromeEnhancedSession : IAsyncDisposable
{
    ...
    private ShareXModChromeEnhancedSession(
    ...
    public static async Task<ShareXModChromeEnhancedSession?> TryCreateAsync(
        IntPtr selectedWindowHandle,
        Rectangle selectedRectangle,
        ShareXModV04Settings settings)
    {
        if (!settings.ChromeEnhancedEnabled || selectedWindowHandle == IntPtr.Zero)
        {
            return null;
        }
        ...
        try
        {
            ...
            ShareXModChromeEnhancedSession session = new(
                client,
                target,
                settings,
                selectedRectangle,
            ...
            await ShareXModChromeLayoutShiftTracker.StartAsync(client, settings);

            session.startSemanticMap =
                await ShareXModChromeSemanticMap.CaptureAsync(
                    client,
                    target,
                    settings,
                    selectedRectangle,
    ...
    public async Task FinalizeSemanticCaptureAsync()
    {
        ...
        try
        {
            endSemanticMap = await ShareXModChromeSemanticMap.CaptureAsync(
                client,
                Target,
                settings,
                selectedRectangle,
        ...
        try
        {
            layoutShiftPath = await ShareXModChromeLayoutShiftTracker.FlushAsync(
                client,
                settings);
        }
        catch
        ...
        try
        {
            await ShareXModSemanticRepairAugmenter.TryAugmentAsync(
                settings,
                startSemanticMap?.Path,
                endSemanticMap?.Path,
                layoutShiftPath,
                startSemanticMap?.Calibration);
        }
        catch
        {
        }
        ...
        {
            repairCapture =
                await ShareXModChromeRepairCapture.CaptureCandidatesAsync(
                    client,
                    settings,
                    startSemanticMap?.Calibration);
        }
    ...
    public async Task<ShareXModRepairAlignmentResult?> ApplyVerifiedRepairsAsync(
        Bitmap? result)
    {
        try
        {
            return await ShareXModRepairAlignmentGate.TryApplyAsync(
                result,
                settings,
                repairCapture);
        }
    ...
    public async Task<ShareXModChromeImageAppendixResult?> ExportImageAppendixAsync(
        string captureDirectory)
    {
        try
        {
            return await ShareXModChromeImageAppendix.ExportAsync(
                client,
                settings,
                captureDirectory);
        }
        ...
        string windowTitle)
    {
        ...
        if (!string.IsNullOrWhiteSpace(windowTitle))
        {
            ...
            ShareXModChromeTarget? exact =
                targets.FirstOrDefault(t => NormalizeTitle(t.Title) == normalizedWindow);
            ...
            if (contains != null &&
                CommonTitleScore(normalizedWindow, NormalizeTitle(contains.Title)) >= 0.45)
            {
                return contains;
            }
        }
