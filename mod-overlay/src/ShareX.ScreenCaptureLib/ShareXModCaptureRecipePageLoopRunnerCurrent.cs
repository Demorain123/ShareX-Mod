...
/// <summary>
/// Current simple Page Template Loop runner. This intentionally leaves the historical v0.8 runner
/// untouched and composes the later safety layers in one place: semantic range anchors, Page Guard,
/// per-page native image collection and committed-boundary resume.
/// </summary>
internal static class ShareXModCaptureRecipePageLoopRunnerCurrent
{
        ...
        Func<bool>? shouldStop = null)
    {
        ...
        if (!plan.Candidate || plan.NextPageStep?.Locator == null) return null;
        ...
        ShareXModPageLoopResumeJournal resumeJournal =
            ShareXModPageLoopResumeJournal.Open(
                settings,
                settings.CaptureRecipeReplayPath,
                suggestedDirectory);

        string directory = resumeJournal.OutputDirectory;
        CleanupUnusedSuggestedDirectory(suggestedDirectory, directory, resumeJournal.IsResumed);
        Directory.CreateDirectory(directory);
        ShareXModCaptureSessionContext.RegisterComponent("capture-recipe-page-loop", directory);
        ShareXModCaptureSessionContext.RegisterComponent("page-loop-resume", directory);

        List<LoopPart> parts = resumeJournal.Parts
            .Select(x => new LoopPart(
                x.File,
                x.PageNumber,
                x.TemplateStep,
                x.PageKey,
        ...
        int resumeStartPage = resumeJournal.IsResumed
            ? Math.Max(1, resumeJournal.NextPageNumber)
            : 1;

        if (resumeJournal.IsResumed)
        {
            (resumeBlocked, resumeStartPage, stopReason) =
        ...
        int loopStartPage = resumeBlocked ? maxPages + 1 : resumeStartPage;

        for (int pageNumber = loopStartPage; pageNumber <= maxPages; pageNumber++)
        {
            if (shouldStop?.Invoke() == true)
            {
                stoppedByUser = true;
                stopReason = "manual-stop";
                // Do not mark the journal complete: the last committed ready/capturing boundary is
                // exactly what the next run needs in order to resume safely.
                break;
            }
            ...
            catch
            {
                ...
                resumeJournal.Complete("failed");
                break;
            }

            string pageStateKey = pageEvidence.Url + "\u001f" + pageEvidence.MainFingerprint;
            if (!seenPageStates.Add(pageStateKey))
            {
                stopReason = "page-loop-cycle-detected";
                resumeJournal.Complete(stopReason);
                break;
            }

            string currentPageKey = PageKeyFromUrl(pageEvidence.Url);
            resumeJournal.BeginPage(pageNumber, pageEvidence);
            ...
            foreach (ShareXModCaptureRecipeStep templateRange in plan.CaptureRanges)
            {
                ...
                ShareXModCaptureRecipeStep effective =
                    await ShareXModRecipeRangeAnchorResolver.ResolveStepAsync(client, pageStep);
            ...
            if (stoppedByUser || failed)
            {
                ...
                if (failed) resumeJournal.Complete("failed");
                break;
            }
            ...
            {
                failed = true;
                stopReason = "page-loop-page-guard-failed";
                pages.Add(new PageAudit(
                    pageNumber,
                    pageEvidence.Url,
                    currentPageKey,
            ...
            List<ShareXModPageLoopResumePart> committedPageParts = parts
                ...
                .Select(x => new ShareXModPageLoopResumePart(
                    ...
                    x.Height))
                .ToList();
            resumeJournal.RecordPageParts(pageNumber, committedPageParts);

            // Capture native assets while this page's DOM/resources are still present. This is
            // important for virtualized lists where previous pages disappear after navigation.
            ...
            resumeJournal.BeforeNext(pageNumber, pageEvidence);
            ...
            {
                resumeJournal.Complete(failed ? "failed" : stopReason);
                break;
            }

            ShareXModRecipePageEvidence nextPageEvidence =
                await ShareXModRecipePageTransitionVerifier.CaptureAsync(client);
            resumeJournal.CompleteTransition(pageNumber, nextPageEvidence);
        ...
        int startPage)
    {
        ...
        {
            journal.NeedsReview(
                "Resume stopped at an ambiguous in-flight Next Page boundary; existing captured pages are preserved.");
            return (true, startPage, "page-loop-resume-needs-review");
        }

        if (journal.Phase.Equals("complete", StringComparison.OrdinalIgnoreCase))
        {
            journal.NeedsReview(
                "A previous runner marked this journal complete; automatic continuation is intentionally not guessed.");
            return (true, startPage, "page-loop-resume-complete-boundary-review");
        }
        ...
        {
            journal.NeedsReview(
                "Current browser page no longer matches the last committed resume boundary.");
            return (true, startPage, "page-loop-resume-page-mismatch");
        }
        ...
        Func<bool>? shouldStop)
    {
        ...
        foreach (ShareXModRecipeStaleRange stale in staleRanges)
        {
            ...
            if (!repaired.Success)
            {
                pageNotes.Add("page-loop-page-guard-failed:" + repaired.Detail);
                return false;
            }

            int templateStep = stale.OriginalStep.Index % 100000;
            List<LoopPart> old = parts
                .Where(x => x.PageNumber == pageNumber && x.TemplateStep == templateStep)
                .ToList();

            foreach (LoopPart oldPart in old)
            {
                parts.Remove(oldPart);
                ShareXModCaptureRecipePageGuardRepair.ArchiveOldPart(directory, oldPart.File);
            }
            ...
            await pageGuard.RecordAsync(stale.OriginalStep, stale.EffectiveStep);
            pageNotes.Add(
                $"page-guard-recaptured-template-step-{templateStep}:{string.Join(',', stale.Reasons)}");
        }

        if (staleRanges.Count == 0) return true;
        ...
        ShareXModV04Settings settings)
    {
        ...
        {
            WriteManifest(
                directory, target, plan, parts, pages,
                stoppedByUser, truncated, failed, stopReason,
                null, 0, 0, navigationCount);
            return null;
        }
        ...
        WriteManifest(
            directory, target, plan, parts, pages,
            stoppedByUser, truncated, failed, stopReason,
            finalPng, sourceWidth, sourceHeight, navigationCount);
        ...
        int navigationCount)
    {
        try
        {
            File.WriteAllText(
                ...
                {
                    format = "ShareX-Mod Capture Recipe Page Loop Current",
    ...
    private static string ResolveDirectory(ShareXModV04Settings settings)
    {
        ...
        string directory = Path.Combine(
            root,
            $"page-loop-current-{DateTime.Now:yyyyMMdd-HHmmss-fff}-p{Environment.ProcessId}");
        Directory.CreateDirectory(directory);
        return directory;
    }
    ...
    private static string PageKeyFromUrl(string url)
    {
        ...
        catch
        {
            byte[] bytes = System.Security.Cryptography.SHA256.HashData(
                Encoding.UTF8.GetBytes(url ?? string.Empty));
            return Convert.ToHexString(bytes.AsSpan(0, 8)).ToLowerInvariant();
        }
    }
}
